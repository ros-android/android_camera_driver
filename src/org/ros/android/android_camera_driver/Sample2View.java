...
class Sample2View extends SampleCvViewBase {
    ...
    protected Bitmap processFrame(VideoCapture capture) {
        ...
        String[] compDescStrings = {"Total processFrame", "Grab a new frame", "MatToBitmap", "Publish cameraInfo",
                "Create ImageMsg", "Compress image", "Transfer to Stream", "Image.SetData", "Publish Image", "Total seconds per frame"};
        ...
        switch (MainActivity.viewMode) {
            case MainActivity.VIEW_MODE_GRAY:
                ...
                capture.retrieve(mFrame, Highgui.CV_CAP_ANDROID_GREY_FRAME);
                //Imgproc.cvtColor(mGray, mFrame, Imgproc.COLOR_GRAY2RGBA, 4);
                break;
            case MainActivity.VIEW_MODE_RGB:
                capture.retrieve(mFrame, Highgui.CV_CAP_ANDROID_COLOR_FRAME_RGBA);
                //Core.putText(mFrame, "OpenCV + Android", new Point(10, 100), 3, 2, new Scalar(255, 0, 0, 255), 3);
                break;
            case MainActivity.VIEW_MODE_CANNY:
                capture.retrieve(mFrame, Highgui.CV_CAP_ANDROID_GREY_FRAME);
                Imgproc.Canny(mFrame, mFrame, 80, 100);
                //Imgproc.cvtColor(mIntermediateMat, mFrame, Imgproc.COLOR_GRAY2BGRA, 4);
                break;
        }
        ...
        if (bmp == null)
            bmp = Bitmap.createBitmap(mFrame.cols(), mFrame.rows(), Bitmap.Config.ARGB_8888);
        ...
        try {
            ...
            if (MainActivity.imageCompression >= MainActivity.IMAGE_TRANSPORT_COMPRESSION_PNG) {
                ...
                if (MainActivity.imageCompression == MainActivity.IMAGE_TRANSPORT_COMPRESSION_PNG)
                    bmp.compress(Bitmap.CompressFormat.PNG, MainActivity.imagePNGCompressionQuality, baos);
                else if (MainActivity.imageCompression == MainActivity.IMAGE_TRANSPORT_COMPRESSION_JPEG)
                    bmp.compress(Bitmap.CompressFormat.JPEG, MainActivity.imageJPEGCompressionQuality, baos);
